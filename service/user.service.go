...
func (us *userService) Login(userReq dto.UserLoginReq) (dto.UserLoginRes, error) {
	...
	accessToken, err := utils.GenerateToken(isUser.Id, string(isUser.Role), isUser.DepartementId)
	if err != nil {
		return dto.UserLoginRes{}, err
	}

	refreshToken, err := utils.GenerateRefreshToken(isUser.Id, string(isUser.Role), isUser.DepartementId)
	if err != nil {
		return dto.UserLoginRes{}, err
	}
